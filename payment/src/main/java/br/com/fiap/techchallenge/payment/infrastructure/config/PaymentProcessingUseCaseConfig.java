...
public class PaymentProcessingUseCaseConfig {
    @Bean
    PaymentProcessingUseCase getPaymentProcessingUseCase(IPaymentGateway paymentGateway, IPaymentRepository paymentRepository, IMessagePublisher messagePublisher) {
        return new PaymentProcessingUseCase(paymentGateway, paymentRepository, messagePublisher);
    }
}
